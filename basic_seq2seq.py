...
# source_data,target_data = bytes(),bytes()
...
def  extract_target_vocab(data):
    ...
    """
    special_words = ['<PAD>','<UNK>','<GO>','<EOS>']
    set_words = list([word for word in data.split('\n')])
    print("target num", len(set_words))
    int_to_vocab = {idx:word for idx,word in enumerate(special_words + set_words)}
    vocab_to_int = {word:idx for idx,word in int_to_vocab.items()}
...
# source_int = [[source_word_to_int.get(word, source_word_to_int['<UNK>']) for seq in line for word in seq] for line in source_data.split('\n')]
...
def pad_sentence_batch(sentence_batch,pad_int): # 需不需要设置为定长? eg. 60
    ...
    - pad_int: <PAD>对应索引号
    '''
    max_sentence = 30 # max([len(sentence) for sentence in sentence_batch])
...
with tf.Session(graph=loaded_graph) as sess:
    ...
    answer_logits = sess.run(logits, {input_data: [text] * batch_size,
                                      target_sequence_length: [len(source_line)] * batch_size,
